...
try:
    ...
    if ta > t1 and ta < t2:
        ...
        if not DT.in_trade(client, "ETHUSDT"):
            ...
            candles = historic.get_data_bybit_DERIVATIVES('ETHUSDT', 'D', 199)
            ...
            if cons[0] == 1:
                # LONG
                if DT.long_tpsl(client, "ETHUSDT", qty, round(1.08 * DT.get_ticker_USDT(client, id), 2), round(0.96 * DT.get_ticker_USDT(client, id), 2), "Market"):
                    print("Buy succesful")
                else:
                ...
                # SHORT
                if DT.short_tpsl(client, "ETHUSDT", qty, round(0.92 * DT.get_ticker_USDT(client, id), 2), round(1.06 * DT.get_ticker_USDT(client, id), 2), "Market"):
