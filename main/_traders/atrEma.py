...
class init:
    ...
    def __init__(self, paper_trading=True, start_money=1000, fee=0.0015, ema1=200, _other=0):
    ...
    def update(self):
        ...
        if self.in_trade and self.paper_trading:
            if self.in_long:
                if self.candles[-1].low <= self.buys[-1] * (2 - self.percentage[-1]):
                    self.money = self.money + self.money * (2 - self.percentage[-1]) * leverage - self.money * self.fee * leverage - self.money * leverage
                    self.in_trade = False
                    self.losses += 1
            ...
            if not self.in_long:
                if self.candles[-1].high >= self.sells[-1] * (2 - self.percentage[-1]):
        ...
        if not self.in_trade and self.i > self.start_after_candles:
            if (self.ema1T[-1] < self.candles[-1].open) and (self.atr_lower_ema[-1] > self.candles[-1].low):
                # BUY
            ...
            elif (self.ema1T[-1] > self.candles[-1].open) and (self.atr_upper_ema[-1] < self.candles[-1].high):
                # SELL
